...
size_t Controller::GetShortestPathBetweenVertices(size_t vertex1, size_t vertex2) {
	return algorithms_.GetShortestPathBetweenVertices(graph_, vertex1, vertex2);
}

std::vector<std::vector<size_t>> Controller::GetShortestPathsBetweenAllVertices() {
...
bool Controller::GraphIsEmpty() {
	return graph_.GraphIsEmpty();
}
