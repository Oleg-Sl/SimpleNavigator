...
private:
    ...
    void exchangeGenes(std::vector<size_t> &genes1, std::vector<size_t> &genes2, Range &range);
    Range generateRandomRange(size_t max_size);
    void inheritGenes(std::vector<size_t> &new_genes, std::vector<size_t> &old_genes, Range &range);
};
