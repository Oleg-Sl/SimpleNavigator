...
Population TournamentSelection::execute(Population &population) {
    ...
    size_t number_participants = std::min(kParticipantCount, population.getSize() - 1);

    while (new_population.getSize() < population.getSize()) {
        std::set<size_t> participant_indexes = generateUniqueRandomNumbers(0, population.getSize() - 1, number_participants);
...
Chromosome TournamentSelection::getWinnerChromosome(Population &population, const std::set<size_t> &participant_indexes) {
...
std::set<size_t> TournamentSelection::generateUniqueRandomNumbers(size_t start, size_t end, size_t count) {
    if (start > end) {
