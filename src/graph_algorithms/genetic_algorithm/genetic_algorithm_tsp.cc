...
GeneticAlgorithmTsp::GeneticAlgorithmTsp(Graph &distances, SelectionStrategy &selection, CrossoverStrategy &crossover, MutationStrategy &mutation) : distances_(distances), selection_(selection), crossover_(crossover), mutation_(mutation) {
};

TsmResult GeneticAlgorithmTsp::solve(size_t number_generations, size_t population_size, double possible_mutation, double possible_crossover) {
    TsmResult min_path;
