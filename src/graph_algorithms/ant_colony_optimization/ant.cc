...
TsmResult &Ant::getPath() {
    return path_;
}


double Ant::getPheromonQuantiy() {
    return pheromon_quantiy_;
}


size_t Ant::getCurrentVertex() {
    return path_.vertices.back();
}


std::vector<double> Ant::getProbabilities(Pheromones &pheromones, std::vector<size_t> &neighbors) {
...
double Ant::calcSummaryWeight(Pheromones &pheromones, std::vector<size_t> &neighbors) {
    size_t current_vertex = getCurrentVertex();
